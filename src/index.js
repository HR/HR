...
async function getStats() {
  ...
  const user = await userInfoFetcher(githubToken).then(
    (res) => res.data.data.viewer
  )
  ...
  stats.totalForks = user.repositories.nodes.reduce((prev, curr) => {
    return prev + curr.forkCount
  }, 0)
  stats.totalWatchers = user.repositories.nodes.reduce((prev, curr) => {
    return prev + curr.watchers.totalCount
  }, 0)
...
async function updateGist(stats) {
  ...
  const gistContent =
    [
      ['⭐', `Stars`, humanize(stats.totalStars)],
  ...
  return request('PATCH /gists/:gist_id', {
    ...
    console.info(
      `Updated Gist ${gistId} with the following content:\n${gistContent}`
    )
  })
}
